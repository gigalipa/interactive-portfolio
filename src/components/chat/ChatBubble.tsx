...
const markdownComponents: Components = {
	...
	a: ({ children, href }) => (
		<a
			...
			target="_blank"
			rel="noreferrer"
			className="text-signal-cyan underline hover:text-signal-cyan/80"
		>
			{children}
		</a>
	),
	...
	code: ({ children, className }) =>
		className?.includes("language-") ? (
			...
			<code className="bg-slate-mist rounded px-1 py-0.5 font-mono text-xs">{children}</code>
		),
	pre: ({ children }) => (
		<pre className="bg-void/60 my-2 overflow-x-auto rounded-lg p-2 font-mono text-xs">{children}</pre>
	),
	h1: ({ children }) => <p className="font-display mt-2 mb-1 font-semibold">{children}</p>,
	h2: ({ children }) => <p className="font-display mt-2 mb-1 font-semibold">{children}</p>,
	h3: ({ children }) => <p className="font-display mt-2 mb-1 font-semibold">{children}</p>,
	blockquote: ({ children }) => (
